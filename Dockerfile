...
RUN echo 'oddsc' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/oddsc.sh

RUN echo 'cd ' >>/oddsc.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/oddsc.sh

RUN echo 'cd /noVNC-1.2.0' >>/oddsc.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/oddsc.sh

RUN chmod 755 /oddsc.sh
